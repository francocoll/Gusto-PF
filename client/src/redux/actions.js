...
export const getAllProducts = () => {
	return async function (dispatch) {
		...
		try {
			//json
			const response = await axios.get("http://gusto-pf-franco.up.railway.app/products");
			return dispatch({
...
export const userProfile = (email) => {
	return async function (dispatch) {
		const json = await axios.get(`http://gusto-pf-franco.up.railway.app/users/email?email=${email}`);
